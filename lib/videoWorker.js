...
let loadedFontManifest = null;

async function getManifest() {
  if (!loadedFontManifest) {
    loadedFontManifest = await fetch('/fonts/manifest.json').then(r => r.json());
  }

  return loadedFontManifest;
}
...
async function loadFont(fontFamily, weight = '400', style = 'normal') {
  ...
  const response = await fetch(`/fonts/${fontFamily}/${fileName.print}/`);
