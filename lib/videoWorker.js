...
onmessage = async (e) => {
  ...
  if (e.data.type === 'DRAW_FRAME') {
    ...
    ctx.fillStyle = e.data.backgroundColour;
    ...
    e.data.objects.forEach(object => {
      ...
      if (object.type === "rectangle") {
        ctx.fillRect(-object.width/2, -object.h/2, object.width, object.h);
...
function erase(eraserTexture, d, eraserOpacity, x, y){
  ...
  console.log('erase', eraserTexture, d, eraserOpacity, x, y)
...
async function loadFont(fontFamily, weight = '400', style = 'normal') {
  const fileName = fontFiles[fontFamily]?.[style]?.[weight];
  ...
  await font.load();
  self.fonts.add(font); // or document.fonts.add(font) in main thread
...
const fontFiles = {
  Poppins: {
    ...
    italic: {
      '100': 'Poppins-ThinItalic.ttf',
      '200': 'Poppins-ExtraLightItalic.ttf',
      '300': 'Poppins-LightItalic.ttf',
      '400': 'Poppins-Italic.ttf',
      '700': 'Poppins-BoldItalic.ttf',
      '800': 'Poppins-ExtraBoldItalic.ttf',
      '900': 'Poppins-BlackItalic.ttf',
    },
  },
  Raleway: {
    normal: {
      '100': 'Raleway-Thin.ttf',
      '200': 'Raleway-ExtraLight.ttf',
      '300': 'Raleway-Light.ttf',
      '400': 'Raleway-Regular.ttf',
      '700': 'Raleway-Bold.ttf',
      '800': 'Raleway-ExtraBold.ttf',
      '900': 'Raleway-Black.ttf',
    },
    italic: {
