...
export const saveJpgMyFiles = async (projectTitle, canvas, dpi, user) => {
 ...
 const file = await new Promise(canvas.toBlob((blob) => {
  // Wrap the blob into a proper File instance
  resolve( new File([blob], 'instance-filename.png', { type: 'image/png' }));
    console.log(customFile.name); // Outputs: "instance-filename.png"
  }, 'image/png'));
  */
