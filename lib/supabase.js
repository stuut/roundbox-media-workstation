...
export async function createBoardWithMembers(boardData, users, workspaceId) {
  // Insert new workplace and get the inserted row
...
export const deleteTasksFromBoard = async (taskIds) => {
  try {
      const { data, error } = await supabase
      .from('tasks')
      .delete()
...
WHERE bt.board_id = 'your_board_id_here'
  ...
  */
  ...
  export async function createTask(taskData, boardId, workspaceId, users) {
    // Insert new workplace and get the inserted row
    ...
    const { error: boardTaskError } = await supabase
      .from('board_tasks')
      .insert([{ task_id: taskId, board_id: boardId}]);
    ...
    const { error: workspaceTaskError } = await supabase
      ...
      .insert([{ task_id: taskId, workspace_id: workspaceId}]);
