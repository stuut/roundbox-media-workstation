...
const getAnimatedProps = (element, localTime) => {
 ...
 if (element.clippingPath && (isPlaying || isTrackingRef.current)){
    const {clipCx, clipCy, clipWidth, clipHeight} = getClippingValues(element)

  //  props.cx = clipCx + props.cx
  //  props.cy = clipCy + props.cy
    ...
    if (textEditing){
      ...
      if (resizing) {
        ...
        if (obj.clippingPath){
          ...
          halfH = Math.abs(localY) + (obj.height/2 - cropHalfH)
      ...
      if (resizingSide) {
        ...
        if (obj.clippingPath){
          ...
          newW = (right - left) + cropW;
          newH = (bottom - top) + cropH;
