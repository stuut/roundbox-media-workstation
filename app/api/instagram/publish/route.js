...
async function publishToInstagram(job, channel) {
  ...
  file?.file_url?.match(/\.(mp4|mov|m4v)$/i);
...
try{
  ...
  const { channelId, publication } = await req.json();
  ...
  const result = await publishToInstagram(publication, channel[0])



  return Response.json({ id:result.id, data:result },{ status: 200 })
