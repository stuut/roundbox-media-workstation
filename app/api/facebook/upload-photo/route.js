import { createClient } from "@supabase/supabase-js"

const supabase = createClient(
  process.env.NEXT_PUBLIC_SUPABASE_URL,
  process.env.SUPABASE_SERVICE_ROLE_KEY
)

export const config = {
  api: {
    bodyParser: {
      sizeLimit: "20mb"
    }
  }
}

export async function POST(req) {

  try {

    const { channelId, endPoint, file, published } = await req.json();

    const { data: channel, error } = await supabase
    .from("platform_accounts")
    .select(`*`)
    .eq("id", channelId)

    if (error) throw error

    if (!channel) {
      return Response.json({ message: "No Social Channel found" },{ status: 500 })
    }

    if (!endPoint) {
      return Response.json({ message: "No Endpoint" },{ status: 500 })
    }

    const accessToken = channel[0]?.access_token

    const socialId = channel[0]?.external_account_id

    const facebookResponse = await fetch(`https://graph.facebook.com/v24.0/${socialId}/${endPoint}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({
          caption: file.description,
          url: file.file_url,
          published : published,
          temporary : true,
          access_token : accessToken,
        }),
      })

      if (!facebookResponse.ok) {
        console.error(facebookResponse);
        return Response.json({ message: `Upload image to facebook failed with status: ${facebookResponse.status}` },{ status: 500 })
      }

      const postResponseJson = await facebookResponse.json();


      return Response.json({ id:postResponseJson.id, data:postResponseJson },{ status: 200 })

  }catch (err){
    console.error(err);
    return Response.json(
      { error: "Failed to Schedule facebook post" },
      { status: 500 }
    );
  }







}
