...
const exportVideoFrames = async (download = true) => {
...
const applyTemplate = (type, template) => {
  ...
  const image = activeScene.getObjectById(postDataRef.current.image)
  const text =  activeScene.getObjectById(postDataRef.current.title)
  ...
  if (type === 'videos'){
    ...
    if ( template === 'Reel'){
     ...
     if (minDuration >= 5){
       //change duration
       onSceneUpdateProperty('duration', Math.round(minDuration+2))
...
const easeInOutCubic = (t) => {
 ...
 const getAnimatedProps = (element, localTime) => {
  ...
  element.animations.forEach(anim => {
    ...
    const progress = Math.max(0, Math.min(1, (localTime - anim.startTime) / anim.duration));
    ...
    if (localTime > anim.startTime + anim.duration) {
      // Animation complete
...
/*
    setScenes(prev =>
      ...
      prev.map(scene => {
        ...
        // mutate objects directly, but return new scene object
        scene.objects.push(newObj)

        return  scene; // new object reference triggers React rerender
      })
...
function drawSoftStrokePreview(stroke) {
  ...
  const removeScene = (activeScene) => {
    ...
    setScenes(prev => prev.filter(scene => scene.id !== activeScene.id))

    sceneManagerRef.current.activeSceneId = null

    sceneManagerRef.current.removeScene(activeScene)
    ...
    <div style={{position:'relative'}}>
      ...
      {open &&
        <div style={{position:'absolute', marginTop: '10px'}} className='canvas-zoom-dropdown dropshadow'>
          ...
          <p onClick={() => exportVideoFrames(true)}>Export Video</p>
          {postInfo&&
            <button onClick={() => showShare(true)} style={{flex: 2}} className='btn secondary icon-button'>
...
if (element === null) return <div></div>
  ...
  return(
    <div>
      <strong><p style={{paddingLeft:'10px'}}>Animate Element</p></strong>
      <hr/>
      <button
    ...
    <div>
      ...
      <div className="flex-1 overflow-x-auto">
        ...
        >
          ...
          {/* Element tracks */}
          <div style={{position:'relative', overflowY: 'scroll', height: '130px', display:'flex', gap:'2px'}}>
                      ...
                      <div key={index}>
                        <div
                          style={{
                            ...
                            borderColor: selectedElement?.id === element.id
                                    ? 'var(--md-sys-color-secondary-container)'
                                    : element?.fill?lightenRgba(element?.fill, .5):'var(--md-sys-color-secondary-container)',
                            alignItems: 'center',
                            borderWidth:'3px',
                            borderStyle: 'solid',
                            boxSizing: 'border-box',
                        ...
                        >
                          ...
                          {(element.type === 'rectangle' || element.type === 'ellipse' || element.type === 'triangle') &&
                            <span style={{color:'#ffffff',paddingLeft:'10px', fontSize:'.9em'}} >
                              {element.type}
                            </span>
                          }
                        </div>
...
const PropertiesPanel = ({
  ...
  return(
    <div>
      <strong><p style={{paddingLeft:'10px'}}>Element Properties</p></strong>
...
const Share = ({
  ...
  const displayVideo = async() => {
    setVideoLoader(true)
    const videoBlob = await exportVideoFrames(false)
