...
if (!image || !text || !webtext) return
  ...
  if (type === 'videos'){
    ...
    if ( template === 'Reel'){
     ...
     handleUpdateElementState(activeSceneState.id, text.id, { animations: text.animations})
     ...
     handleUpdateElementState(activeSceneState.id, image.id, { animations: image.animations})
    }
  ...
  setActiveElement(null)
  selectedIndexRef.current = null
  setCurrentTime(activeSceneState.start)
  drawLower()
  drawArtboard()
  drawUpper()
}
  ...
  */
  ...
  const drawArtboard = () => {
    ...
    scene.elements.forEach(obj => {
      ...
      const animationProps = getAnimatedProps(obj, localTime);
      ...
      if (obj.type === "rectangle") {
        ...
        if (textHilightRef.current){
          ...
          if (obj.clippingPath){
            ctx.rect(
              obj.clippingPath.left * scaleRef.current,
              obj.clippingPath.top * scaleRef.current,
              (obj.clippingPath.right - obj.clippingPath.left) * scaleRef.current,
              (obj.clippingPath.bottom - obj.clippingPath.top) * scaleRef.current
            );
            // 3. Apply the clip
            ctx.clip();
          }
      ...
      if (obj.type !== "pen" && obj.type !== "image"){
...
const drawUpper = () => {
  ...
  if (selectedIndex != null) {
    ...
    const scale = scaleRef.current * animatedProps.scale;
    ...
    ctx.beginPath();
    ctx.arc(screenLeft+(screenW/2), +screenH / 2 + ROTATE_DISTANCE, HANDLE_SIZE * 2, 0, 2 * Math.PI);
    ctx.closePath();
    ctx.stroke();
    ...
    // Curved arrow
    ctx.beginPath();
    ctx.arc(screenLeft+(screenW/2), +screenH / 2 + ROTATE_DISTANCE, HANDLE_SIZE / 1.2, Math.PI * 0.10, Math.PI * 1.7);
    ctx.strokeStyle = activeToolRef.current === 'cropping'? CROP_COLOUR : TRANSFORM_COLOUR;
    ctx.lineWidth = TRANSFORM_WIDTH;
    ctx.stroke();
    ...
    scene.elements.forEach(object => {
      ...
      if (object.type === "rectangle") {
        ...
        if (object.clippingPath && (isPlaying || isTrackingRef.current)){
          ...
          const left   = object.clippingPath.left;
          const right  = object.clippingPath.right;
          const top    = object.clippingPath.top;
          const bottom = object.clippingPath.bottom;
          ...
          const newCx = animationProps.cx + offsetX;
          const newCy = animationProps.cy + offsetY;
          ...
          ctx.drawImage(
            object.img,
            -object.width / 2 - (left + right) / 2,
            -object.h / 2 - (top + bottom) / 2,
            object.width,
            object.h
          );
    ...
    if (textEditing){
      ...
      if (resizing) {
        ...
        if (obj.clippingPath){
          ...
          const padX = obj.width/2 - oldW/2;
          const padY = obj.h/2 - oldH/2;
      ...
      if (resizingSide) {
        ...
        const obj = getActiveElement()
        const animated = getAnimatedProps(obj);
        const oldLeft   = -obj.width / 2;
        const oldRight  =  obj.width / 2;
        const oldTop    = -obj.h / 2;
        const oldBottom =  obj.h / 2;
        ...
        if (obj.clippingPath){
          ...
          const cropWidth  = obj.clippingPath.right - obj.clippingPath.left
          const cropHeight  = obj.clippingPath.bottom - obj.clippingPath.top

          diffX = obj.width - cropWidth
          diffY = obj.h- cropHeight
        ...
        if (obj.clippingPath){
          ...
          obj.clippingPath.width = clipWidth
          obj.clippingPath.height = clipHeight
        ...
        if (!obj.clippingPath){
          localCx = (left + right) / 2
          localCy = (top + bottom) / 2;
        }else{
            const cropW = obj.clippingPath.right - obj.clippingPath.left;
             const cropH = obj.clippingPath.bottom - obj.clippingPath.top;

             const diffX = obj.width - cropW;
             const diffY = obj.h - cropH;
        ...
        const worldDx =
          localCx * Math.cos(animated.angle) -
          localCy * Math.sin(animated.angle);

        const worldDy =
          localCx * Math.sin(animated.angle) +
          localCy * Math.cos(animated.angle);
        ...
        {activeScene &&
          <div className='properties-container'>
            ...
            <div className="property-label" style={{marginTop:0}}><Film className="property-icon" />
                <p>Active Scene</p>
                <Plus style={{marginLeft:'auto'}} onClick={()=>createScene(activeScene.duration,5,true)} />
            </div>
            ...
            {[...activeScene.elements].reverse().map((element, index)=>{
            ...
            <div className='col-2 column-gap-2'>
              <button onClick={() => duplicateScene(activeScene)} style={{flex: 4, marginBottom:0}} className='btn secondary icon-button'>
                <Copy className='button-icon'/>
                Duplicate
              </button>
              {sceneManagerRef.current?.scenes.length>1&&
              <button style={{marginBottom:0}} onClick={() => removeScene(activeScene)} className='btn danger'><Trash2 style={{verticalAlign: 'middle'}} className="h-3 w-3"/></button>
              }
            </div>
          </div>
        }
