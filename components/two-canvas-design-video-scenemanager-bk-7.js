        ...
        if (textHilightRef.current){
          ...
          if (object.clippingPath && (isPlaying || isTrackingRef.current)){
            ...
            ctx.save();
              ...
              ctx.rect(
                (-(object.clippingPath.right - object.clippingPath.left)/2)  * scaleRef.current,
                (-(object.clippingPath.bottom - object.clippingPath.top)/2)  * scaleRef.current,
    ...
    scene.elements.forEach(object => {
      ...
      if (object.type === "rectangle") {
        ...
        if (object.clippingPath && (isPlaying || isTrackingRef.current)){
          ...
          ctx.save();
            ctx.translate(object.cx + animationProps.cx, object.cy + animationProps.cy);
            ctx.rotate(animationProps.angle);
            ctx.scale(animationProps.scale, animationProps.scale);
...
function hitPolygon(px, py, polygon) {
  ...
  const handleMouseMove = (e) => {
    ...
    if (textEditing){
      ...
      if (resizing) {
        ...
        const oldClipW = clip.right - clip.left??null
        const oldClipH = clip.bottom - clip.top??null
        const oldClipCx = (clip.left + clip.right) / 2??null
        const oldClipCy = (clip.top + clip.bottom) / 2??null
        ...
        if (obj.clippingPath){
           ...
           if(obj.clippingPath.left + obj.clippingPath.right !== 0 || obj.clippingPath.top + obj.clippingPath.bottom !== 0){
             ...
             // redraw offset clipping path proportional
             ...
             const clipCx = (clip.left + clip.right) / 2;
              ...
              clip.bottom = clipCy + (clip.bottom - clipCy) * scaleY;
               ...
               // redraw 1:1 clipping path proportional
  ...
  // 1️⃣ Transform mouse → local object space
        ...
        if (obj.clippingPath) {
          ...
          switch (side) {
            case 0: clip.left   = localX; break; // left
            case 1: clip.right  = localX; break; // right
            case 2: clip.top    = localY; break; // top
            case 3: clip.bottom = localY; break; // bottom
          }
