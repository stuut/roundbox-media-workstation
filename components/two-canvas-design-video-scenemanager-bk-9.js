...
const onSelectScene = (id) => {
  ...
  init() {
    ...
    console.log("This runs only on creation!");
  }
        ...
        if (object.clippingPath && (isPlaying || isTrackingRef.current)){
          ...
          ctx.save();
            ...
            if (object.clippingPath){
              ...
              ctx.rect(
                object.clippingPath.left * scaleRef.current,
                object.clippingPath.top * scaleRef.current,
...
const drawUpper = () => {
  ...
  if (selectedIndex != null) {
    ...
    if (object.clippingPath && (isPlaying || isTrackingRef.current)){
      ...
      screenCx = offset.x + object.clippingPath.cx + animatedProps.cx * scaleRef.current;
      screenCy = offset.y + object.clippingPath.cy + animatedProps.cy * scaleRef.current;

    }else{

      screenCx = offset.x + cx * scaleRef.current;
      screenCy = offset.y + cy * scaleRef.current;
    }


    const scale = scaleRef.current * animatedProps?.scale??0;

    const screenLeft   = left * scale;
    const screenRight  = right * scale;
    const screenTop    = top * scale;
    const screenBottom = bottom * scale;
    ...
    scene.elements.forEach(object => {
      ...
      if (object.type === "rectangle") {
        ...
        if (object.clippingPath){
          ...
          ctx.save();
            ctx.translate(object.clippingPath.cx + animationProps.cx, object.clippingPath.cy + animationProps.cy);
            ctx.rotate(animationProps.angle);
            ctx.scale(animationProps.scale, animationProps.scale);
...
function hitPolygon(px, py, polygon) {
    ...
    if (textEditing){
      ...
      if (resizing) {
        ...
        if (obj.clippingPath){
          ...
          // Normalie size as if you are reszing from image edge not from crop edge
          halfW = Math.abs(localX) + (obj.width/2 - cropHalfW)
          halfH = Math.abs(localY) + (obj.h/2 - cropHalfH)
        }else{
        ...
        if(obj.clippingPath.left + obj.clippingPath.right !== 0 || obj.clippingPath.top + obj.clippingPath.bottom !== 0){
         }else{
           obj.width= halfW * 2; obj.h = halfH * 2;
         }
         if (obj.clippingPath){
           let scaleX
           let scaleY // non proportional cropping
           if(obj.clippingPath.left + obj.clippingPath.right !== 0 || obj.clippingPath.top + obj.clippingPath.bottom !== 0){
             const oldClipW = obj.clippingPath.right - obj.clippingPath.left;
             const oldClipH = obj.clippingPath.bottom - obj.clippingPath.top;
             const oldClipCx = (obj.clippingPath.left + obj.clippingPath.right) / 2;
             const oldClipCy = (obj.clippingPath.top + obj.clippingPath.bottom) / 2;
             const clipCx = (obj.clippingPath.left + obj.clippingPath.right) / 2;
             const clipCy = (obj.clippingPath.top + obj.clippingPath.bottom) / 2;
             halfW = Math.abs(localX - clipCx);
             halfH = Math.abs(localY - clipCy);
             const aspect = oldClipW / oldClipH;
             if (!keepRatio) {
               const newAspect = halfW / halfH;
               if (newAspect > aspect) halfW = halfH * aspect;
               else halfH = halfW / aspect;
             }
             scaleX = halfW*2 / oldClipW;
             scaleY = halfH*2 / oldClipH;

             console.log('non proportional cropping')

             obj.width *= scaleX;
             obj.h *= scaleY;
           }else{
             scaleX = obj.width / oldWidth;
             scaleY = obj.h / oldHeight;
           }
           obj.clippingPath.left *= scaleX;
           obj.clippingPath.right *= scaleX;
           obj.clippingPath.top *= scaleY;
           obj.clippingPath.bottom *= scaleY;
           obj.clippingPath.cx = obj.cx
           obj.clippingPath.cy = obj.cy
         }
  ...
  // 1️⃣ Transform mouse → local object space
        const dx = pos.x - obj.cx;
        const dy = pos.y - obj.cy;
        ...
        if (obj.clippingPath) {
          ...
          let scaleX = newClipW / oldClipW;
          let scaleY = newClipH / oldClipH;

          // Update object size to keep the extra padding outside clip
    ...
    // dragging element
      ...
      if (resizing) {
        ...
        if (obj.clippingPath){
          ...
          oldClipW = clip.right - clip.left;
          oldClipH = clip.bottom - clip.top;
          oldClipCx = (clip.left + clip.right) / 2;
          oldClipCy = (clip.top + clip.bottom) / 2;
        ...
        //non proportional scale because an offcenter clip exisits
        if (offCenterClip){
          ...
          const scaleX = halfW*2 / oldClipW;
          const scaleY = halfH*2 / oldClipH;

          obj.clippingPath.left   = clipCx + (clip.left   - clipCx) * scaleX;
          obj.clippingPath.right  = clipCx + (clip.right  - clipCx) * scaleX;
          obj.clippingPath.top    = clipCy + (clip.top    - clipCy) * scaleY;
          obj.clippingPath.bottom = clipCy + (clip.bottom - clipCy) * scaleY;

          const clippingValues = getClippingValues(obj)

          obj.clippingPath.width = obj.clippingPath.right - obj.clippingPath.left
          obj.clippingPath.height = obj.clippingPath.bottom - obj.clippingPath.top
          obj.clippingPath.cx = clippingValues.clipCx
          obj.clippingPath.cy = clippingValues.clipCy
      ...
      if (resizingSide) {
        ...
        switch (side) {
          ...
          const oldLocalCx = (clip.left + clip.right) / 2;
          const oldLocalCy = (clip.top + clip.bottom) / 2;
          ...
          const shiftX = newLocalCx - oldLocalCx;
          const shiftY = newLocalCy - oldLocalCy;

          // Rotate local center shift back to world space
          const worldDx =
          shiftX * Math.cos(animated.angle) -
          shiftY * Math.sin(animated.angle);

          const worldDy =
          shiftX * Math.sin(animated.angle) +
          shiftY * Math.cos(animated.angle);
