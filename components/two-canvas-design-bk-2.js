...
//  const [dragging, setDragging] = useState(null);
  ...
  const [animateCallBack, setAnimateCallBack] = useState(false);
...
drawTextChars(ctx, animationProps) {
  ...
  // Check if there are line-based animations
        ...
        if (hasLineAnim) {
          // Render each line with its own animation
          /*
          lines.forEach((line, i) => {
            ctx.save();

            // Get line-specific animation properties
            const lineAnimProps = getLineAnimatedProps(element, i, lines.length);
            ctx.globalAlpha = animProps.opacity * lineAnimProps.opacity;

            ctx.fillText(
              line,
              animProps.x + lineAnimProps.x,
              animProps.y + (i * (element.fontSize || 48) * 1.2) + lineAnimProps.y
            );
          ...
          /*
          lines.forEach((line, i) => {
            ctx.fillText(line, animProps.x, animProps.y + (i * (element.fontSize || 48) * 1.2));
          });*/
        }
...
const getLineAnimatedProps = useCallback((element, lineIndex, totalLines) => {
    ...
    element.animations?.forEach(anim => {
      ...
      if (currentTime < lineStartTime) {
        // Animation hasn't started for this line yet
      ...
      if (currentTime > lineEndTime) {
        // Animation complete for this line
        return;
      }
  ...
  const handleMouseUp = () => {
    ...
    if (selectedIndexRef.current !== null){
      ...
      if (!["pen", "air brush"].includes(obj.type)){
        updateActiveElement({ cy: obj.cy, y: obj.y, cx: obj.cx, x: obj.x, width:obj.width, h:obj.h})
        handleUpdateElements( obj.id, { cy: obj.cy, y: obj.y, cx: obj.cx, x: obj.x, width:obj.width, h:obj.h})
      }
...
function drawSoftStrokePreview(stroke) {
  ...
  const removeItem = (index, obj) => {
    objectsRef.current.splice(index, 1);
    selectedIndexRef.current = null
    setActiveElement(null)
    console.log('setActiveElement(null)')
    ...
    >
      ...
      <div className='canvas-left-side-toolbar dropshadow'>
        ...
        <Tool
          ...
          >
            <div>
              <Media user={user} addImages={addImages}/>
            </div>
        </Tool>
        <FillColourPicker
          //callBack={toolCallback}
        ...
        />
        {(animateCallBack && activeElement)&&
          <AnimatePanel element={activeElement} onUpdateElement={onUpdateElementAnimations} duration={duration}/>
        }
      </div>
      ...
      {activeElement &&
        <div
          ...
          top: (offsetRef.current.y + activeElement.cy * scale) - (activeElement.h / 2 * scale),
          background:'#fffff',
          ...
          <div style={{margin: '0px 0px 0px 15px', width:130}}>
            <Animatebutton setAnimateCallBack={setAnimateCallBack} animateCallBack={animateCallBack}/>
          </div>
        </div>
...
const handleFileFunction = async (data) => {
  ...
  return(
    <>
      <button className='btn primary' onClick={() => addImages(selectedFiles)}>{`Add File${selectedFiles.length>1?'s':''}`}</button>
      <input
    ...
    <div style={{
      ...
      {files.map((file, index)=>{
        return (
          <div key={file.id} style={{width:'48%', margin:'1%'}}>
            <div>
...
const Animatebutton = ({setAnimateCallBack, animateCallBack}) => {
...
useEffect(()=>{
  ...
  setAnimateCallBack(activeAnimate)
...
useEffect(()=>{
  ...
  setActiveAnimate(animateCallBack)
    ...
    <div className="h-48 bg-slate-900/95 backdrop-blur-xl border-t border-slate-700/50 flex flex-col">
      ...
      <div className="flex-1 overflow-x-auto">
        ...
        >
          ...
          {/* Element tracks */}
          <div className="py-2 px-0" style={{position:'relative', overflowY: 'scroll', height: '100%'}}>
