...
const drawUpper = () => {
  ...
  if (selectedIndex != null) {
    ...
    if (object.clippingPath && (isPlaying || isTrackingRef.current)){
      screenCx = offset.x + object.clippingPath.cx + cx * scaleRef.current;
      screenCy = offset.y + object.clippingPath.cy + cy * scaleRef.current;
    }else{
    ...
    // dragging element
      ...
      if (resizing) {
        ...
        if (offCenterClip){
          ...
          clip.left   = clipCx + (clip.left   - clipCx) * scaleX;
          clip.right  = clipCx + (clip.right  - clipCx) * scaleX;
          clip.top    = clipCy + (clip.top    - clipCy) * scaleY;
          clip.bottom = clipCy + (clip.bottom - clipCy) * scaleY;
