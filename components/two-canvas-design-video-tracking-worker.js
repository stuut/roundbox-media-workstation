...
useEffect(() => {
  ...
  const offscreen = lowerRef.current.transferControlToOffscreen();
...
const changeTime = (time) => {
  ...
  const otherItems = objectsRef.current.filter(item => item.id !== activeElement.id);
...
useEffect(()=>{
  offscreenCanvasRef.current.width = PAGE_WIDTH + BLEED * 2
  offscreenCanvasRef.current.hight = PAGE_HEIGHT + BLEED * 2
  createBufferCanvas()
},[PAGE_WIDTH, PAGE_HEIGHT, BLEED])
...
const onSelectElement = (id) => {
  ...
  constructor({
    ...
    if (this.text && typeof offscreenCanvasRef?.current?.getContext === 'function') {
  ...
  updateLinesWrap() {
    ...
    const ctx = trackingBufferRef.current.getContext('2d');
  ...
  updateLines() {
    const lower = offscreenCanvasRef.current;
    if (!lower) return;
...
redrawImage(ctx, animationProps) {
  ...
  if (this.clippingPath){
     ...
     let screenW = this.clippingPath.width
     let screenH = this.clippingPath.height

     const widthDiff = this.width - this.clippingPath.width
     const heightDiff = this.h - this.clippingPath.height

     if (this.clippingPath.side === 1){
      screenW = screenW + widthDiff
    }

    ctx.rect(-screenW / 2, -screenH / 2, this.clippingPath.width, this.clippingPath.height);
...
drawTextChars(ctx, animationProps) {
  ...
  lines.forEach((line, lineIndex) => {
    ...
    for (let charIndex = 0; charIndex < line.length; charIndex++) {
      ...
      console.log('hasTextAnim', hasTextAnim)
      if (hasTextAnim) {
...
const applyTemplate = (type, template) => {
  ...
  if (type === 'videos'){
    ...
    if ( template === 'Reel'){
      ...
      text.width = PAGE_WIDTH - 200
      text.cx = PAGE_WIDTH/2
      text.cy = PAGE_HEIGHT/2
...
const onDrop = async(e) => {
  if (dragMedia){
    if (dragMedia.file_type === 'video/mp4' || dragMedia.file_type === 'video/webm'){
      ...
      //addImage({file_url:dragMedia.data.heroImage.fields.file.url})

      loadPost({
        image:dragMedia.website === 'www.hilltopsphoenix.com.au'?dragMedia.data.heroImage.fields.file.url:dragMedia.data.image.fields.file.url,
        title:dragMedia.data.title,
        website:dragMedia.website
      })
 ...
 const getAnimatedProps = (element) => {
  ...
  element.animations.forEach(anim => {
    ...
    if (currentTimeRef.current < anim.startTime) {
       ...
       if (anim.type === 'slideInLeft' ) props.x = element.x - 300;
       if (anim.type === 'slideInRight') props.x = element.x + 300;
       if (anim.type === 'slideInTop') props.y = element.y - 300;
       if (anim.type === 'slideInBottom') props.y = element.y + 300;
...
function hitPolygon(px, py, polygon) {
  ...
  const handleMouseUp = async() => {
    ...
    if (isPaintingRef.current && paintType === 'air brush'){
      ...
      const bufferBitmap = await createImageBitmap(bufferRef.current)

      obj.airbrushBuffer = bufferRef.current;
      obj.brushOpacity = brushOpacity;
      obj.airbrushBufferBitmap = bufferBitmap
      ...
      await drawBuffer(bufferBitmap, brushOpacity)
...
function drawBrushPreview(element, paintType) {
  console.log('drawBrushPreview')

  if (paintType === 'pen'){
    drawPen(element.points, element.fill, brushSize);
  }else if (paintType === 'air brush'){
...
const eraseAt = async(x, y) => {
  ...
  const eraserBitmap = await createImageBitmap(eraserTextureRef.current)

  console.log('eraserBitmap', eraserBitmap)

  console.log('eraserTextureRef.current.width', eraserTextureRef.current.width)

  workerRef.current.postMessage({
    type: 'ERASE',
    eraserTexture: eraserBitmap,
    d: eraserTextureRef.current.width,
    eraserOpacity: eraserOpacity/100,
    x:x,
    y:y
  });
...
const drawBuffer = (buffer, brushOpacity) => {
  return new Promise((resolve) => {
    const handler = (e) => {
      if (e.data.type === 'DRAW_DONE') {
...
function drawPen(points, colour = "rgba(0,0,0,1)", brushSize = 50) {
  ...
  const removeItem = (obj) => {
    ...
    selectedIndexRef.current = null
    setActiveElement(null)
    setElements(prev => prev.filter((object) => object.id !== obj.id))
...
useEffect(()=>{
  if (paintType === 'air brush'){
    brushTextureRef.current = createBrushTexture();
    if (!offscreenCanvasRef.current || !overlayRef.current) return;
    const buffer = document.createElement('canvas');
    const bufferCtx = buffer.getContext('2d');
    buffer.width = offscreenCanvasRef.current.width
    buffer.height = offscreenCanvasRef.current.height
...
useEffect(()=>{
  ...
  if (activeTool === 'eraser'){
    ...
    delta.width = offscreenCanvasRef.current.width
    delta.height = offscreenCanvasRef.current.height
...
projectTitle
  ...
  return(
    <div style={{position:'relative'}}>
      ...
      {open &&
        <div style={{position:'absolute', marginTop: '10px'}} className='canvas-zoom-dropdown dropshadow'>
          <p onClick={() => saveAsPng(projectTitle, canvas, 300)}>Save as PNG</p>
          <p onClick={() => saveAsjpg(projectTitle, canvas, 300)}>Save as JPG</p>
          <p onClick={() => exportVideo('webm', true)}>Export Webm</p>
          <p onClick={() => exportVideo('mp4', true)}>Export mp4</p>
        </div>
      }
    </div>
  )
...
const updateAnimationSelect = (index, updates) => {
    ...
    if (!allowedTypes.has(updates.type)){
      ...
      console.log('newAnimation.duration')
          ...
          <div className="py-2 px-0" style={{position:'relative', overflowY: 'scroll', height: '95px'}}>
            {[...elements].reverse().map((element, index) => {
                ...
                return(
                  <div key={index}>
                    <div
                      className={`h-10 mb-1 rounded-lg cursor-pointer flex items-center text-xs font-medium transition-all no-highlight`}
                      style={{
                        marginTop: '10px',
                    ...
                    >
                      ...
                      {element.type === 'text'&&
                        <span style={{color:'#ffffff',paddingLeft:'10px'}} className="truncate">
...
const FeedsPanel = ({
    ...
    const onFeedChange = (value) => {
      ...
      const selectedFeed = FEEDS.find(item => item.label === value);
    ...
    useEffect(()=>{
        ...
        if (selectedFeed && dateFilter){
          getFeed()
        }

    },[selectedFeed, dateFilter])
    ...
    const getFeed = async () => {
        ...
        const posts = response.items.filter((item)=>{
          return item.fields.publishDate === date
        })
          ...
          return(
            <div key={index} style={{width:'48%'}}>
              <img
                ...
                draggable
                onDragStart={() => onDragStart({type:'post',data:post.fields,website:selectedFeed.website})}
    ...
    <div>
      ...
      <div style={{padding:'0px 10px'}}>
        <div className='col-2 column-gap-2'>
          ...
          <button style={{}} onClick={() => removeItem(element)} className='btn danger'><Trash2 style={{verticalAlign: 'middle'}} className="h-3 w-3"/></button>
        </div>
        {element.type === 'text' &&
          <div>
            <Tool
