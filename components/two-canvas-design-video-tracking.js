...
const exportVideoFrames = async (download = true) => {
  ...
  return new Promise(async(resolve) => {
    ...
    for (let frame = 0; frame < totalFrames; frame++) {
         ...
         drawLower(true); // your canvas render
         //drawArtboard()
    ...
    const res = await fetch('/api/encode-video-frames', { method: 'POST', body: formData });
...
const clearCanvas = (ctx) => {
  ...
  const drawLower = (exportVideo = false) => {
    ...
    objectsRef.current.forEach(object => {
      ...
      object.currentTime = currentTimeRef.current
      const animationProps = getAnimatedProps(object);
    ...
    >
      ...
      {activeElement &&
        <div className={`side_menu_right dropshadow`}>
          <div className='col-2 column-gap-2'>
            <button onClick={() => duplicate(activeElement)} style={{flex: 2}} className='btn secondary icon-button'>
              <Copy className='button-icon'/>
              Duplicate
            </button>
            <button style={{}} onClick={() => removeItem(aciveElement)} className='btn danger'><Trash2 style={{verticalAlign: 'middle'}} className="h-3 w-3"/></button>
          </div>
          <div className='col-2 column-gap-2'>
...
if (element === null) return <div></div>
  ...
  return(
    <div>
      <strong><p style={{paddingLeft:'10px'}}>Element Animate</p></strong>
      <hr/>
      <button
      ...
      <div className='col-2 column-gap-2' style={{height:'100%'}}>
        ...
        <div style={{position:'relative'}}>
          <div style={videoLoader? {display:'block'}:{display:'none'}} className={'loader_screen'}>
              <div style={{transform:'translate(-50%, -50%)'}}  className="loader"></div>
              <div style={{transform:'translate(-50%, -50%)'}}>{`Creating Video ${progress}%`}</div>
